...
async function TriggerNuiCallback<T = unknown>(callback: string, data?: unknown, mockData?: T): Promise<T> {
    ...
    const resource = (window as any).GetParentResourceName ? (window as any).GetParentResourceName() : 'fivem_solidjs_boilerplate';
    const response = (await fetch(`https://${resource}/${callback}`, options)).json();
    return response;
};

export default TriggerNuiCallback;